...
const Dashboard = () => {
  ...
  const editTitle = async (event) => {
    try {
      event.preventDefault()
      const {data} = await
      api.put(`/api/resumes/update`, {resumeId: editResumeId,resumeData: {title}},
      { headers: { Authorization: `Bearer ${token}` } }
    )
  ...
  const deleteResume = async (resumeId) => {
    ...
    if (confirm) {
      const {data} = await
      api.delete(`/api/resumes/delete/${resumeId}`, 
      { headers: { Authorization: `Bearer ${token}` } }
    )
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 sm:flex flex-wrap gap-4">
          {allResumes.map((resume, index) => {
            const baiseColor = colors[index % colors.length];
            return (
              <button
                key={index}
                ...
                style={{
                  background: `linear-gradient(135deg, ${baiseColor}10, ${baiseColor}40)`,
                  borderColor: baiseColor + "40",
                }}
              >
                <FilePenLineIcon
                  className="size-7 group-hover:scale-105 transition-all"
                  style={{ color: baiseColor }}
                />
                <p
